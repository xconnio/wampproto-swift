...
class SessionTests: XCTestCase {
    ...
    func testReceiveError() throws {
        ...
        let callErr = Error(messageType: Call.id, requestID: call.requestID, uri: "wamp.error.invalid_argument")
        ...
        let publishErr = Error(messageType: Publish.id, requestID: publish.requestID, uri: "wamp.error.invalid_uri")
    ...
    func testExceptions() throws {
        ...
        let invalidError = Error(messageType: Register.id, requestID: 10, uri: "wamp.error.procedure_already_exists")
        ...
        XCTAssertThrowsError(try session.receive(data: serializer.serialize(
            message: Error(messageType: Registered.id, requestID: 100, uri: "wamp.error.invalid_argument"))))

        // receive error invalid Call id
        XCTAssertThrowsError(try session.receive(data: serializer.serialize(
            message: Error(messageType: Call.id, requestID: 100, uri: "wamp.error.invalid_argument"))))

        // receive error Register id
        XCTAssertThrowsError(try session.receive(data: serializer.serialize(
            message: Error(messageType: Register.id, requestID: 100, uri: "wamp.error.invalid_argument"))))

        // receive error invalid Unregister id
        XCTAssertThrowsError(try session.receive(data: serializer.serialize(
            message: Error(messageType: Unregister.id, requestID: 100, uri: "wamp.error.invalid_argument"))))

        // receive error invalid Subscribe id
        XCTAssertThrowsError(try session.receive(data: serializer.serialize(
            message: Error(messageType: Subscribe.id, requestID: 100, uri: "wamp.error.invalid_argument"))))

        // receive error invalid Unsubscribe id
        XCTAssertThrowsError(try session.receive(data: serializer.serialize(
            message: Error(messageType: Unsubscribe.id, requestID: 100, uri: "wamp.error.invalid_argument"))))

        // receive error invalid Publish id
        XCTAssertThrowsError(try session.receive(data: serializer.serialize(
            message: Error(messageType: Publish.id, requestID: 100, uri: "wamp.error.invalid_argument"))))
    }
